...
class SlimmableResNet(nn.Module):
    def __init__(self, block, layers, out_indices=(0, 1, 2, 3)):
        ...
        self.conv1 = SlimmableConv2d(
            [3] * len(self.width_mults), self.inplanes, 
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(layers):
            ...
            if i in self.out_indices:
                # Handle width multiplier with padding if needed
                if self.width_mult < 1.0:
                    bs = x.size()
                    padding_channels = int((1-self.width_mult) * (bs[1]/self.width_mult))
                    y = torch.cat([x, torch.zeros(bs[0], padding_channels, bs[2], bs[3], device=x.device)], 1)
                else:
                    y = x
                outs.append(y)
...
def SlimmableResNet18(out_indices=(0, 1, 2, 3)):
    return SlimmableResNet(Block, [2,2,2,2], out_indices)
    
def SlimmableResNet34(out_indices=(0, 1, 2, 3)):
    return SlimmableResNet(Block, [3,4,6,3], out_indices)      
        
def SlimmableResNet50(out_indices=(0, 1, 2, 3)):
    return SlimmableResNet(Bottleneck, [3,4,6,3], out_indices)
    
def SlimmableResNet101(out_indices=(0, 1, 2, 3)):
    return SlimmableResNet(Bottleneck, [3,4,23,3], out_indices)

def SlimmableResNet152(out_indices=(0, 1, 2, 3)):
    return SlimmableResNet(Bottleneck, [3,8,36,3], out_indices)