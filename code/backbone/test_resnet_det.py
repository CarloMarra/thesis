...
from resnet_det import SlimmableResNet34
...
def count_active_params(model, width_mult):
    ...
    for _, module in model.named_modules():
        if isinstance(module, torch.nn.Conv2d) and hasattr(module, 'current_in_channels'):
            # How many parameters are actually "in use" under this width
...
def test_slimmable_resnet():
    """Test the SlimmableResNet18 at multiple width multipliers."""
    print("Initializing SlimmableResNet34...")
    model = SlimmableResNet34()  # Return all 4 stages
    device = 'mps' if torch.backends.mps.is_available() else 'cpu'
    model = model.to(device)
    
    batch_size = 16  # Lower or raise as needed
    ...
    for width in FLAGS.width_mult_list:
        ...
        # Make sure outputs is a tuple (stages)
        if not isinstance(outputs, tuple):
            outputs = (outputs, )
        
        # For each stage, show shape & active channels
        for i, feat_map in enumerate(outputs):
            # Count channels that actually have non-zero values
            # (If you use zero-padding, some last channels may be zero.)
            active_channels = (feat_map.abs().sum(dim=[0, 2, 3]) > 0).sum().item()
            print(f"  Stage {i+1} output shape: {feat_map.shape}, "
                  f"Active channels: {active_channels}/{feat_map.shape[1]}")
    ...
    print("\n=== Summary of results ===")
    # Assume the widest is the "baseline"
